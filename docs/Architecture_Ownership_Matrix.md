...
**Site Explorer boundary (mandatory).** The AIC schema — including the derived
attack-surface routing projection field — is **unchanged** for W7-B
compatibility. However, qa-automation SHALL NOT produce security-applicability
decisions from the AIC, interpret it as a security recommendation, classify
security applicability, recommend attack targets, or assign vulnerability
probability. qa-automation populates only the QA/application-intelligence fields
it owns; the attack-surface projection remains `NOT_PRODUCED` here (W7-E) and is
produced and interpreted solely by downstream Skillmatrix security authorities.
