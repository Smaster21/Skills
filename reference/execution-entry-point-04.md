<!-- PRESERVED VERBATIM (part 4 of 6). Split ONLY to satisfy the 200-line
...
**Constraint:** read-only (`03` §36) — navigation and disclosure activation are
permitted; form submission and destructive/write-intent controls are not.
Depth-first, exhaustive, **no caps**. Never delete data, create accounts, submit
payments, bypass authentication, or store unmasked secrets.
See `site-agnostic-discovery.md` and `coverage-ledger.md`.
...
logic, backend defects, API failures, auth failures, or data corruption
(`08` §13). A healed locator is a candidate; promotion needs
